...
void LaunchTransposeDim1And2<paddle::platform::float16>(
                            ...
                            const paddle::platform::float16* input,
  ...
  Dim3 input_dims_aligned = {
      input_dims[0],
      framework::CeilOrFloor<int, true>(input_dims[1], kTileY),
  ...
  TilingSwapDim1And2Diagonal<paddle::platform::float16,
        kBlockDimX, kBlockDimY, kTileX, kTileY, kPaddingSize>
        <<<grids, threads, 0, d.stream()>>>(
        input, input_dims, output);
}
