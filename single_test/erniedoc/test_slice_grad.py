...
# paddle
pd_in = paddle.to_tensor(data=x_in, dtype='float32', place=paddle.CUDAPlace(0), stop_gradient=False)
...
pd_res = out_pd.numpy()
out_pd.backward()
pd_dx = pd_in.grad.numpy()

# tensorflow
with tf.device('/device:GPU:0'):
  tf_x = tf.constant(x_in, dtype=tf.float32)
