...
__global__ void KeMatMulSimple(float* A, float* B, float* C
                             int m, int k, int n) {
...
__global__ void KeMatrixMultiplyShared(float* A, float* B, float* C,
                                        int m, int k, int n) {
...
//normal version
__global__ void KeMatrixMultiplyNormal(float* A, float* B, float* C,
                                        int m, int k, int n) {
    int row = threadIdx.y, col = threadIdx.x;
    if(row < m && col < n) {
        __shared__ float s_a[TILE][TILE];
