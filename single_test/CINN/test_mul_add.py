...
def reader(limit):
    for i in range(limit):
        yield np.random.random(size=(128, 16)).astype('float32'), \
              np.random.random(size=(16, 16)).astype('float32'),  \
...
def RandFeedData(loop_num=10):
  ...
  for i in range(loop_num):
    x, w, b = next(data)
    feed.append({'x' : x, 'weight' : w, 'bias' : b})
  return feed

def BuildProgram(main_program, startup_program):
  with paddle.static.program_guard(main_program, startup_program):
    x = paddle.static.data(name='x', shape=[None, 16], dtype='float32')
    weight = paddle.static.data(name='weight', shape=[16, 16], dtype='float32')
    bias = paddle.static.data(name='bias', shape=[None, 16], dtype='float32')

    hidden = paddle.fluid.layers.mul(x, weight)
    prediction = paddle.fluid.layers.elementwise_add(hidden, bias)
...
if __name__ == "__main__":
  ...
  assert(np.allclose(np.asarray(loss_f), np.asarray(loss_t), atol=1e-5))
  # '''
