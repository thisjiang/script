...
static GtkWidget *frame, *vbox, *box2,
    *table; /*box2用来封装文本构件与垂直滚动条*/
...
GnomeUIInfo client_operation_menu[] = {
    ...
    GNOMEUIINFO_ITEM_NONE("退出", "退出程序", gtk_main_quit), GNOMEUIINFO_END};
/*生成航班查询菜单项*/
GnomeUIInfo inquire_menu[] = {
    GNOMEUIINFO_ITEM_NONE("查询特定航班", "查询某一特定航班机票信息",
                          inquireone),
...
GnomeUIInfo help_menu[] = {
    ...
    GNOMEUIINFO_ITEM_NONE("关于", "关于此程序说明", about), GNOMEUIINFO_END};
/*生成顶层菜单项*/
GnomeUIInfo menubar[] = {
    GNOMEUIINFO_SUBTREE("客户端操作(_S)", client_operation_menu),
    GNOMEUIINFO_SUBTREE("机票查询(_Q)", inquire_menu),
    GNOMEUIINFO_SUBTREE("帮助(_H)", help_menu), GNOMEUIINFO_END};
/*生成工具栏*/
GnomeUIInfo toolbar[] = {
    GNOMEUIINFO_ITEM_STOCK("连接服务器", "与远程服务器建立连接", connectserver,
                           GNOME_STOCK_PIXMAP_PRINT),
    GNOMEUIINFO_ITEM_STOCK("断开连接", "断开与远程服务器的连接", disconnect,
                           GNOME_STOCK_PIXMAP_CUT),
    GNOMEUIINFO_ITEM_STOCK("购买机票", "购买机票", buyticket,
                           GNOME_STOCK_PIXMAP_OPEN),
    GNOMEUIINFO_ITEM_STOCK("查询特定航班", "查询某一特定航班机票信息",
                           inquireone, GNOME_STOCK_PIXMAP_SAVE),
    GNOMEUIINFO_ITEM_STOCK("查询所有航班", "查询所有航班机票信息", inquireall,
                           GNOME_STOCK_PIXMAP_SAVE),
    GNOMEUIINFO_ITEM_STOCK("退出", "退出程序", gtk_main_quit,
...
int button_buyticket(GnomeDialog *dialog, gint id, gpointer data) {
  ...
  if (id == 0) { //如果“确定”按钮被点击
    ...
    if (flight_ID <= 0 ||
        flight_ID >
            10) { //判断输入的航班号是否正确，不正确的话，给出提示信息，重新输入。
      mbox = gnome_message_box_new("输入的航班号错误！请重新输入！",
    ...
    if (ticket_num <=
        0) { //判断输入的票数是否正确，不正确的话，给出提示信息，重新输入。
      mbox = gnome_message_box_new("输入的票数错误！请重新输入！",
                                   GNOME_MESSAGE_BOX_INFO,
                                   GNOME_STOCK_BUTTON_OK, NULL);
      gtk_widget_show(mbox);
      gtk_window_set_modal(GTK_WINDOW(mbox), TRUE);
    ...
    /*发送出错*/
    if (ret == -1) {
      mbox = gnome_message_box_new("发送失败！请重新发送！",
                                   GNOME_MESSAGE_BOX_INFO,
                                   GNOME_STOCK_BUTTON_OK, NULL);
      gtk_widget_show(mbox);
      gtk_window_set_modal(GTK_WINDOW(mbox), TRUE);
    ...
    if (ret == -1) {
      mbox = gnome_message_box_new("接收失败！请重新发送！",
                                   GNOME_MESSAGE_BOX_INFO,
                                   GNOME_STOCK_BUTTON_OK, NULL);
      gtk_widget_show(mbox);
      gtk_window_set_modal(GTK_WINDOW(mbox), TRUE);
...
void buyticket(GtkWidget *widget, gpointer data) {
  ...
  dialog = gnome_dialog_new(_("购买机票"), GNOME_STOCK_BUTTON_OK,
  ...
  gtk_box_pack_start(GTK_BOX(GNOME_DIALOG(dialog)->vbox), flight_entry, FALSE,
                     FALSE, 0);
  ...
  g_signal_connect(G_OBJECT(dialog), "clicked", G_CALLBACK(button_buyticket),
                   &st_flight); //设定“确定”“取消”按钮的回调函数
  gtk_window_set_modal(GTK_WINDOW(dialog), TRUE);
  gtk_widget_show(dialog);
  gnome_dialog_set_parent(GNOME_DIALOG(dialog),
                          GTK_WINDOW(app)); //设定对话框的父窗口为主程序窗口
}
...
void inquireone() {
  ...
  GtkWidget *dialog;
  GtkWidget *label, *flight_entry;
  dialog = gnome_dialog_new(_("查询特定航班机票信息"), GNOME_STOCK_BUTTON_OK,
                            GNOME_STOCK_BUTTON_CANCEL, NULL);
  ...
  g_signal_connect(G_OBJECT(dialog), "clicked", G_CALLBACK(button_inquireone),
                   flight_entry); //设定“确定”“取消”按钮的回调函数
  gtk_window_set_modal(GTK_WINDOW(dialog), TRUE);
  gtk_widget_show(dialog);
  gnome_dialog_set_parent(GNOME_DIALOG(dialog),
                          GTK_WINDOW(app)); //设定对话框的父窗口为主程序窗口
...
void inquireall() {
  ...
  if (ret == -1) {
    mbox =
        gnome_message_box_new("发送失败！请重新发送！", GNOME_MESSAGE_BOX_INFO,
                              GNOME_STOCK_BUTTON_OK, NULL);
    gtk_widget_show(mbox);
    gtk_window_set_modal(GTK_WINDOW(mbox), TRUE);
  ...
  if (ret == -1) {
    mbox =
        gnome_message_box_new("接收失败！请重新发送！", GNOME_MESSAGE_BOX_INFO,
                              GNOME_STOCK_BUTTON_OK, NULL);
    gtk_widget_show(mbox);
    gtk_window_set_modal(GTK_WINDOW(mbox), TRUE);
    gnome_dialog_set_parent(GNOME_DIALOG(mbox), GTK_WINDOW(app));
...
void displaycontents(GtkWidget *widget, gpointer data) {
  GtkWidget *dialog;
  GtkWidget *label;
  char msg[5][512] = {"连接服务器：与远程服务器建立连接",
                      "断开连接：断开与远程服务器的连接", "购买机票：购买机票",
                      "特定航班查询：查询某一特定航班机票信息",
                      "所有航班查询：查询所有航班机票信息"};
...
int main(int argc, char *argv[]) {
  ...
  g_signal_connect(G_OBJECT(app), "delete_event", G_CALLBACK(gtk_main_quit),
                   NULL);
  //添加菜单和工具栏
  ...
  gtk_text_view_set_editable(GTK_TEXT_VIEW(clientwindow),
                             FALSE);              //文本信息不可编辑
  scrolled = gtk_scrolled_window_new(NULL, NULL); /*创建滚动窗口构件*/
  gtk_scrolled_window_add_with_viewport(
      GTK_SCROLLED_WINDOW(scrolled),
      clientwindow); /*将文本视图构件加入滚动窗口*/
