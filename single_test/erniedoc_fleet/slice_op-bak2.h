 ...
 private:
  ...
  void SliceCompute(const framework::ExecutionContext& context) const {
    ...
    if(in->numel() <= Eigen::NumTraits<int>::highest()) {
      // similar to tf.slice:
      // if element number less than INT_MAX, change the type of index to 32-bit
      ...
      if(need_pad_num == 0) {
        ...
        */
        ...
        if(pad_dim == D - 1) {
          ...
          // convert std::vector array to DDim
          ...
          // the second dimension do not need padding, set padding[0] zero
