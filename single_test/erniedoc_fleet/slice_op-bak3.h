...
class SliceKernel : public framework::OpKernel<T> {
 ...
 private:
  ...
  void SliceCompute(const framework::ExecutionContext& context) const {
    ...
    EigenSlicingCompute(context, in, in_dims, out, out_dims, offsets, extents);

    out->Resize(out_dims);
  }

  template <size_t D>
  void EigenSlicingCompute(
              const framework::ExecutionContext& context,
              const framework::Tensor *in,
              const framework::DDim &in_dims,
              framework::Tensor *out,
              const framework::DDim &out_dims,
              const Eigen::array<int64_t, D> &offsets,
              const Eigen::array<int64_t, D> &extents) const {
    if(D <= 3) {
      // if dimension less than 3, cannot reduce dimension
      LaunchEigenSlicing(context, in, in_dims, out, out_dims, offsets, extents);
    } else { // else we can reduce dimension
      // count not-zero slice number, and record the dimension
      int need_slice_num = 0, slice_dim = -1;
      for(size_t i = 0; i < D; i ++) {
        if(offsets[i] != 0 || extents[i] != in_dims[i]) {
          need_slice_num ++;
          slice_dim = i;
        }
      }
      ...
      if(need_slice_num == 0) {
        // do not need slice, pass if data address same, else copy
        if(out->mutable_data<T>(context.GetPlace()) == in->data<T>()) {
      ...
      } else if(need_slice_num == 1) {
        // only need slice one dimension, we can reduce dimension
        // only the slice dimension is available for us
        /*
        how to reduce dimension(5 to 3 for example):
        before(D=5):
        in_dims:        [x1,  x2,  x3,  x4,  x5]
        offsets:        [0,   0,   a,   0,   0]
        extents:        [x1,  x2,  b,  x4,  x5]
                            | |
                            V V
        after(D=3):
        reshaped_in_dims: [x1*x2,  x3,  x4*x5]
        reshaped_offsets: [0,      a,     0]
        reshaped_extents: [x1*x2,  b,  x4*x5]
        */

        if(slice_dim == D - 1) {
          // only last dimension need slice,
          // reshape the dimension of tensor in 2: [preceding, slicing]
          std::vector<int64_t> in_dims_toreshape(2, 1), out_dims_toreshape(2, 1);
          auto reshaped_offsets = Eigen::array<int64_t, 2>({1, 1});
          auto reshaped_extents = Eigen::array<int64_t, 2>({1, 1});
          ...
          for(int i = 0; i < slice_dim; i ++) {
            ...
            reshaped_offsets[0] *= offsets[i];
            reshaped_extents[0] *= extents[i];
          }
          // second dimension is the slice dimension
          in_dims_toreshape[1] = in_dims[slice_dim];
          out_dims_toreshape[1] = out_dims[slice_dim];

          reshaped_offsets[1] = offsets[slice_dim];
          reshaped_extents[1] = extents[slice_dim];
          ...
          LaunchEigenSlicing(
              context, in, reshaped_in_dims, out,
              reshaped_out_dims, reshaped_offsets, reshaped_extents);
        } else if(slice_dim == 0) {
          // only first dimension need slice,
          // reshape the dimension of tensor in 2: [slicing, succeeding]
          // similar to (D - 1)
          std::vector<int64_t> in_dims_toreshape(2, 1), out_dims_toreshape(2, 1);
          auto reshaped_offsets = Eigen::array<int64_t, 2>({1, 1});
          auto reshaped_extents = Eigen::array<int64_t, 2>({1, 1});
          ...
          in_dims_toreshape[0] = in_dims[slice_dim];
          out_dims_toreshape[0] = out_dims[slice_dim];

          reshaped_offsets[0] = offsets[slice_dim];
          reshaped_extents[0] = extents[slice_dim];
          // sencond dimension is the accumulate of succeeding dimension
          for(size_t i = slice_dim + 1; i < D; i ++) {
            in_dims_toreshape[1] *= in_dims[i];
            out_dims_toreshape[1] *= out_dims[i];

            reshaped_offsets[1] *= offsets[i];
            reshaped_extents[1] *= extents[i];
          }
          ...
          // other dimension need slice
          // reshape the dimension of tensor in 3:
          // [preceding, slicing, succeeding]
          std::vector<int64_t> in_dims_toreshape(3, 1), out_dims_toreshape(3, 1);
          auto reshaped_offsets = Eigen::array<int64_t, 3>({1, 1, 1});
          auto reshaped_extents = Eigen::array<int64_t, 3>({1, 1, 1});
          ...
          for(size_t i = slice_dim + 1; i < D; i ++) {
            ...
            reshaped_offsets[2] *= offsets[i];
            reshaped_extents[2] *= extents[i];
          }
      ...
      if(need_pad_num == 0) {
        ...
        */
        ...
        if(pad_dim == D - 1) {
          ...
          LaunchEigenPadding(
              context, d_input, reshaped_in_dims, d_out,
