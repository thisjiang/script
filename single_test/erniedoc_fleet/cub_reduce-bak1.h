...
#define CUB_REDUCE_RANK_CASE(i, ...)                              \
  ...
  if (rank == reduce_rank) {
    LaunchCubReduceKernel<Tx, Ty, ReduceOp, TransformOp, BlockDim>(
        x_data, y_data, place, reducer,
        transformer, init, reduce_num, stream);
    return;
  }
