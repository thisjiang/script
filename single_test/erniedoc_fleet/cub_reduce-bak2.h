...
struct ConversionFunctor {
  ...
  HOSTDEVICE explicit inline ConversionFunctor(
        const TransformOp& transformer)
...
struct TensorReduceFunctor {
  ...
  void apply() const {
    ...
    TensorReduce<Tx, Ty, ReduceOp, ConversionFunctor<Tx, Ty, TransformOp>>(
        x, y, origin_reduce_dims, init_cast, reducer, ConversionFunctor<Tx, Ty, TransformOp>(transformer), stream);
  }
};
