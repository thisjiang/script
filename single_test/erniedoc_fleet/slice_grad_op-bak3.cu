...
__global__ void KePaddingEigen(const T *input_data,
                              ...
                              const EigenPairArray<D> paddings,
                              T *output_data,
...
template<typename T, size_t D>
void LaunchSlicePaddingKernel(const gpuStream_t &stream,
                              const T *input_data,
                              const EigenIndexArray<D> &in_stride_array,
                              const int64_t in_size,
                              const EigenPairArray<D> &paddings,
  ...
  KePaddingEigen<T, D><<<grids, threads, 0, stream>>>(
      ...
      paddings, output_data, out_stride_array);
}
...
#pragma unroll
  ...
  LaunchSlicePaddingKernel<T, D>(
                dev_ctx.stream(), d_out->data<T>(),
                out_stride_array, out_size, paddings,
                d_input->mutable_data<T>(context.GetPlace()),
                in_stride_array, in_size);
}
...
#pragma unroll
  ...
  if(pad_size >= 100) {
      LaunchSlicePaddingFunction<float16, D>(context, d_input, in_dims, 
                                             d_out, out_dims, paddings);
  } else {
      LaunchEigenFunction<D>(context, d_input, in_dims, 
                             d_out, out_dims, paddings);
  }
}
}  // namespace operators
