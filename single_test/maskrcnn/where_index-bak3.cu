...
__global__ void KeSetTrueIndex(int64_t *out_ptr, const T *cond_data,
                        ...
                        const int64_t rank, const int64_t *true_num_array) {
  const int64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
  extern __shared__ int64_t s_stride[];
  if(threadIdx.x < rank) s_stride[threadIdx.x] = ptr_stride[threadIdx.x];
  __syncthreads();
  ...
  for(int64_t idx = tid; idx < numel; idx += gridDim.x * blockDim.x) {
    if(CheckTrue<T>()(cond_data[idx])) {
        ...
        for (int j = 0; j < rank; j++) {
          int64_t out_index = index / s_stride[j];
          out_ptr[true_index * rank + j] = out_index;
          index -= out_index * s_stride[j];
        }
    }
  }
}
 ...
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    int64_t threads = std::min(
            static_cast<int64_t>(dev_ctx.GetMaxThreadsPerBlock()), numel);
    ...
    //TODO(jiangcheng):write prefix-sum kernel instead of thrust::inclusive_scan
    thrust::inclusive_scan(thrust::device, ptr_true_num_array, 
                          ptr_true_num_array + numel, 
                          ptr_true_num_array);
    KeSetTrueIndex<T><<<grids, threads, rank * sizeof(int64_t), dev_ctx.stream()>>>
