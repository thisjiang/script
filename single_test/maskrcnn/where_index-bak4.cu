...
class CUDAWhereIndexKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &context) const override {
    ...
    auto d_tmp_mem = 
        memory::Alloc(dev_ctx, (numel + rank) * sizeof(int64_t));
    auto h_tmp_mem =
    ...
    cub::DeviceScan::InclusiveSum(nullptr, cub_tmp_size,
                                  ptr_true_num, ptr_true_num,
    ...
    KeGetTrueNum<T><<<grids, threads, 0, dev_ctx.stream()>>>
            (cond_data, numel, ptr_true_num);
    // calculate prefix sum
    cub::DeviceScan::InclusiveSum(ptr_mem, cub_tmp_size,
                                  ptr_true_num, ptr_true_num,
                                  numel, dev_ctx.stream());
    // calculate stride
    h_stride[rank - 1] = 1;
    ...
    memory::Copy(platform::CPUPlace(), h_num,
                 ...
                 ptr_true_num + numel - 1, sizeof(int64_t),
                 dev_ctx.stream());
    dev_ctx.Wait();
